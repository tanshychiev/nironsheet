...
export default function App() {
  ...
  async function upscaleSelected(mode, label) {
    if (!selected) {
      setMessage("Select an artwork first.");
      return;
    }
    ...
    setBusy(true);
    const progressMessage = `Upscaling with ${label}...`;
    setMessage(progressMessage);
    startTaskProgress(progressMessage);

    try {
      const data = await readJson(
        await fetch(api.upscaleItem(selected.id), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      await loadSheet();
      const doneMessage = data.message || `Upscale finished (${label}).`;
      finishTaskProgress(doneMessage);
      setMessage(doneMessage);
    } catch (error) {
      if (taskProgressTimerRef.current) {
      ...
      setTaskProgress(null);
      setMessage(error.message || "Upscale failed.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function exportPng() {
    ...
    setBusy(true);
    const progressLabel = `Smart Clear export · ${exportDpi} DPI`;
    setMessage(`Preparing ${progressLabel}...`);
    startTaskProgress(progressLabel);

    try {
      const response = await fetch(
        `${api.exportPng}?dpi=${exportDpi}&quality=smart`,
      );
      ...
      const aiEnhanced = Number(
        response.headers.get("X-Niron-AI-Enhanced") || 0,
      );
      const engines = response.headers.get("X-Niron-Engines") || "high-quality render";
      const outputWidth = response.headers.get("X-Niron-Export-Width");
      const outputHeight = response.headers.get("X-Niron-Export-Height");

      downloadBlob(
        blob,
        `${sheet.name || "niron-sheet"}_${exportDpi}dpi_smart-clear.png`,
      );

      const doneMessage = aiEnhanced > 0
        ? `Export complete · ${aiEnhanced} layer(s) AI-enhanced · ${engines}`
        : `Export complete · ${outputWidth} × ${outputHeight}px · source quality preserved`;
  ...
  async function exportPsd() {
    ...
    setBusy(true);
    const progressLabel = `Layered PSD export · ${exportDpi} DPI`;
    setMessage(`Preparing ${progressLabel}...`);
    startTaskProgress(progressLabel);
    ...
    try {
      ...
      for (const item of items) {
        const image = await loadImageElement(item.upscaled_url || item.image_url);
      ...
      finishTaskProgress("Layered PSD exported.");
  ...
  return (
    <div className="builder-app niron-pro-ui">
      <style>{DIRECT_EDIT_CSS}</style>
      <header className="topbar pro-topbar">
        <div className="brand-block pro-brand-block">
          <div className="brand-logo">NR</div>
          <div>
            <strong>Niron Print Studio</strong>
            <span>Professional gang-sheet workspace</span>
          </div>
        </div>

        <nav className="pro-menu" aria-label="Main actions">
          <button type="button" onClick={openNewSheetDialog}>New</button>
          <button type="button" onClick={saveCurrentSheet} disabled={busy}>Save</button>
          <span className="pro-menu-separator" />
          <button type="button" onClick={undoLastAction} disabled={!historyStack.length}>Undo</button>
          <button type="button" onClick={() => applySelection(items.map((item) => item.id), items.at(-1)?.id ?? null)} disabled={!items.length}>Select All</button>
          <button type="button" onClick={() => applySelection([])} disabled={!selectionCount}>Deselect</button>
        </nav>

        <div className="topbar-summary pro-document-summary">
          <b>{sheet?.name || "Loading..."}</b>
          <span>
            {sheetWidthCm.toFixed(2)} × {sheetHeightCm.toFixed(2)} cm · {items.length} layers
          </span>
        </div>
      </header>

      <main className="workspace">
        <aside className="sidebar left-sidebar pro-workflow-panel">
          <div className="sidebar-heading">
            <span>WORKFLOW</span>
            <small>Fast print preparation</small>
          </div>
          ...
          <section className="panel-section pro-panel-section">
            ...
            />
            <button
              className="primary-button pro-primary-action"
              onClick={() => fileInputRef.current?.click()}
              disabled={busy}
            >
              + Add Artwork
            </button>
            <p className="muted-text compact-help">
          ...
          <section className="panel-section pro-panel-section">
            ...
            <button
              ...
              disabled={selectionCount !== 1 || busy}
            >
              Edit Artwork
            </button>
            <button
              className="primary-button perfect-clear-button"
              onClick={() => upscaleSelected("perfect_clear", "Perfect Clear AI")}
              disabled={busy || selectionCount !== 1}
            >
              ✦ Perfect Clear AI
            </button>
          ...
          <details className="advanced-disclosure">
            ...
            <div className="advanced-disclosure-body">
              <div className="advanced-grid">
                <button onClick={() => upscaleSelected("hd_1080", "HD 1080")} disabled={busy || selectionCount !== 1}>1080</button>
                <button onClick={() => upscaleSelected("2k", "2K")} disabled={busy || selectionCount !== 1}>2K</button>
                <button onClick={() => upscaleSelected("4k", "4K")} disabled={busy || selectionCount !== 1}>4K</button>
                <button onClick={() => upscaleSelected("print_300dpi", "300 DPI")} disabled={busy || selectionCount !== 1}>300 DPI</button>
                <button onClick={() => upscaleSelected("print_600dpi", "600 DPI")} disabled={busy || selectionCount !== 1}>600 DPI</button>
                <button onClick={() => upscaleSelected("logo_text", "Logo/Text")} disabled={busy || selectionCount !== 1}>Logo</button>
              </div>

              <label className="check-row advanced-check-row">
                <input
                  type="checkbox"
                  checked={allowRotate}
                  onChange={(event) => setAllowRotate(event.target.checked)}
                />
                Rotate 90° while packing
              </label>
          ...
          <div className="workflow-metrics">
            ...
            <Metric label="Waste" value={metrics ? `${metrics.waste_percent}%` : "—"} />
          </div>
        </aside>

        <section className="canvas-column main-screen-editor">
          <div className="canvas-toolbar pro-canvas-toolbar">
            <div className="toolbar-cluster zoom-cluster">
              <button type="button" onClick={() => zoomSheetOut()} title="Zoom out (Ctrl + -)">−</button>
              <label className="sheet-zoom-input-wrap">
                <input
                  ...
                  onChange={(event) => setSheetZoom(numberValue(event.target.value, 100) / 100)}
                  title="Zoom 5% to 50000%"
                />
                <span>%</span>
              </label>
              <button type="button" onClick={() => zoomSheetIn()} title="Zoom in (Ctrl + +)">+</button>
              <button type="button" onClick={resetSheetZoom}>100%</button>
            </div>

            <span className="toolbar-divider" />

            <div className="toolbar-cluster">
              <button type="button" onClick={fitSheetToViewport}>Fit Sheet</button>
              <button type="button" onClick={fitSelectionToViewport} disabled={!selectionCount}>Fit Selection</button>
            </div>

            <div className="toolbar-context">
              <span className="active-tool-chip">{activeMainTool === "move" ? "Move" : activeMainTool}</span>
              <span>{selectionCount ? `${selectionCount} selected` : "No selection"}</span>
              <small>Space + drag to pan · Ctrl + wheel to zoom</small>
            </div>
          </div>
        ...
        <aside className="sidebar right-sidebar pro-inspector">
          ...
          <div className="inspector-content">
              ...
              <>
                <section className="panel-section pro-panel-section">
                  ...
                  {!selected ? (
                    <div className="empty-inspector-state">
                      ...
                      <span>Select artwork on the canvas to edit its properties.</span>
                    </div>
                  ) : selectionCount > 1 ? (
                    <div className="group-selection-summary">
                      <strong>{selectionCount} layers selected</strong>
                      <span>Drag one selected layer to move the group.</span>
                      <button type="button" className="secondary-button" onClick={() => applySelection([])}>Deselect All</button>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="property-grid">
                        <label>
                          ...
                          <input
                            ...
                            onChange={(event) => changeSelectedSize("width_cm", event.target.value)}
                            onKeyDown={(event) => { if (event.key === "Enter") saveSelectedSize(); }}
                          />
                        </label>
                        <label>
                          <span>Height (cm)</span>
                          <input
                            ...
                            onChange={(event) => changeSelectedSize("height_cm", event.target.value)}
                            onKeyDown={(event) => { if (event.key === "Enter") saveSelectedSize(); }}
                          />
                        </label>
                      </div>

                      <div className="ratio-segmented-control">
                        <button
                          type="button"
                          className={selected.lock_ratio ? "active" : ""}
                      ...
                      <div className="action-row">
                        <button onClick={saveSelectedSize}>Apply Size</button>
                        <button className="secondary-button" onClick={() => selectedAction(api.rotateItem, "Rotating...", "Layer rotated 90°.")}>Rotate 90°</button>
                      </div>
                    </>
                  )}
                </section>
                ...
                <section className="panel-section pro-panel-section">
                  ...
                  <div className="segmented-stack">
                    <button disabled={selectionCount !== 1} onClick={() => switchSource("original")}>Original</button>
                    <button disabled={selectionCount !== 1 || !selected?.processed_url} onClick={() => switchSource("processed")}>Cleaned</button>
                    <button disabled={selectionCount !== 1 || !selected?.upscaled_url} onClick={() => switchSource("upscaled")}>Upscaled</button>
                  </div>
                </section>
                ...
                <section className="panel-section pro-panel-section">
                  ...
                  <div className="property-grid">
                    <label>
                      <span>Width (cm)</span>
                      <input type="number" min="1" step="0.10" value={sheetWidthDraft} onChange={(event) => setSheetWidthDraft(event.target.value)} />
                    </label>
                    <label>
                      <span>Height (cm)</span>
                      <input type="number" min="1" step="0.10" value={sheetHeightDraft} onChange={(event) => setSheetHeightDraft(event.target.value)} />
                    </label>
                  </div>
                  <button type="button" onClick={saveManualSheetSize} disabled={busy}>Apply Sheet Size</button>
                  <div className="action-row">
                    <button type="button" className="secondary-button" onClick={expandSheetToArtwork} disabled={busy || !items.length}>Expand</button>
                    <button type="button" className="secondary-button" onClick={cropSheetToArtwork} disabled={busy || !items.length}>Crop</button>
                  </div>

                  <label className="sheet-color-label">Sheet color</label>
                  <div className="sheet-color-swatches pro-sheet-swatches">
            ...
            {rightPanelTab === "layers" && (
              <section className="panel-section pro-panel-section layers-tab-panel">
                <div className="panel-title-row">
                  <h3>Layers</h3>
                  <span>{items.length}</span>
                </div>
                <div className="layers-panel pro-layers-panel">
                  {layerGroups.map((group) => {
                    const everySelected = group.ids.every((id) => selectedIdSet.has(String(id)));
                    const expanded = expandedLayerGroups[group.key] ?? group.items.length === 1;

                    return (
                      <div key={group.key} className="layer-group-card pro-layer-group-card">
                        <div className="layer-group-topline">
                          <button type="button" className="layer-group-toggle" onClick={() => toggleLayerGroup(group.key)}>
                            {expanded ? "▾" : "▸"}
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            <span>{group.items.length > 1 ? "Group" : "Layer"} · {group.name}</span>
                          ...
                          <div className="layer-group-children">
                            {group.items.map((layer, layerIndex) => (
                              <button
                                key={layer.id}
                                type="button"
                                className={`layer-row ${selectedIdSet.has(String(layer.id)) ? "active" : ""}`}
                                onClick={() => applySelection([layer.id], layer.id)}
                              >
                                <span>Layer {layerIndex + 1}</span>
                                <small>{round2(layer.width_cm)} × {round2(layer.height_cm)}</small>
                              </button>
                            ))}
                          </div>
                        )}
                ...
                <button className="danger-button inspector-delete-button" onClick={() => deleteSelected(true)} disabled={!selectionCount || busy}>
                  {selectionCount > 1 ? `Delete ${selectionCount} Layers` : "Delete Selected Layer"}
                </button>
              </section>
            )}

            {rightPanelTab === "export" && (
              <section className="panel-section pro-panel-section export-tab-panel">
                <h3>Export for printing</h3>
                <label>Resolution</label>
                <select value={exportDpi} onChange={(event) => setExportDpi(Number(event.target.value))}>
                  <option value={150}>150 DPI — draft</option>
                  <option value={300}>300 DPI — Smart Clear print</option>
                  <option value={600}>600 DPI — ultra (short sheets)</option>
                </select>
                <button className="green-button export-primary" onClick={exportPng} disabled={busy || !items.length}>
                  Export Smart Clear PNG
                </button>
                <button className="secondary-button" onClick={exportPsd} disabled={busy || !items.length}>
                  Export Layered PSD
                </button>
                <div className="export-summary-card">
                  <span>Document</span>
                  <b>{sheetWidthCm.toFixed(2)} × {sheetHeightCm.toFixed(2)} cm</b>
                  <span>{items.length} layers · {exportDpi} DPI · AI enhances low-resolution layers automatically</span>
                </div>
              </section>
            )}
          </div>
